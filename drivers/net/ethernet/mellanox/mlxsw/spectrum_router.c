			    ...
			    const struct mlxsw_sp_fib6_entry *fib6_entry)
{
	...
	list_for_each_entry(mlxsw_sp_rt6, &fib6_entry->rt6_list, list) {
		struct fib6_nh *fib6_nh = fib6_info_nh(mlxsw_sp_rt6->rt);
		struct in6_addr *gw;
		int ifindex, weight;
...
mlxsw_sp_nexthop6_group_hash(struct mlxsw_sp_fib6_entry *fib6_entry, u32 seed)
{
	...
	list_for_each_entry(mlxsw_sp_rt6, &fib6_entry->rt6_list, list) {
		dev = fib6_info_nh_dev(mlxsw_sp_rt6->rt);
		val ^= dev->ifindex;
	}
		     ...
		     const struct mlxsw_sp_rt6 *mlxsw_sp_rt6)
{
	...
	for (i = 0; i < nh_grp->count; i++) {
		...
		if (nh->rif && nh->rif->dev == fib6_nh->fib_nh_dev &&
...
mlxsw_sp_fib6_entry_offload_set(struct mlxsw_sp_fib_entry *fib_entry)
{
	...
	if (fib_entry->type == MLXSW_SP_FIB_ENTRY_TYPE_LOCAL ||
		...
		rt = list_first_entry(&fib6_entry->rt6_list,
				      struct mlxsw_sp_rt6, list)->rt;
		fib6_nh = fib6_info_nh(rt);
					...
					enum mlxsw_sp_ipip_type *ret)
{
	...
	return fib6_nh->fib_nh_dev &&
	       mlxsw_sp_netdev_ipip_type(mlxsw_sp, fib6_nh->fib_nh_dev, ret);
}
				  ...
				  struct fib6_info *rt)
{
	...
	nh->nh_grp = nh_grp;
	nh->nh_weight = fib6_nh->fib_nh_weight;
	memcpy(&nh->gw_addr, &fib6_nh->fib_nh_gw6, sizeof(nh->gw_addr));
	mlxsw_sp_nexthop_counter_alloc(mlxsw_sp, nh);
