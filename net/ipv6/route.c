...
static struct fib6_info *find_match(struct fib6_info *rt, int oif, int strict,
				   ...
				   bool *do_rr)
{
	const struct fib6_nh *fib6_nh = rt->fib6_nh;
	int m;
					   ...
					   struct net_device *dev)
{
	...
	for_each_fib6_node_rt_rcu(fn) {
		if (rt->fib6_nh->fib_nh_dev->ifindex != ifindex)
			continue;
		if (!(rt->fib6_flags & RTF_ROUTEINFO) || !rt->fib6_nh->fib_nh_has_gw)
			continue;
...
static int rt6_nexthop_info(struct sk_buff *skb, const struct fib6_nh *nh,
			    unsigned int *flags, bool skip_oif)
{
	...
	if (nh->fib_nh_has_gw) {
		if (nla_put_in6_addr(skb, RTA_GATEWAY, &nh->fib_nh_gw6) < 0)
...
static int rt6_add_nexthop(struct sk_buff *skb, const struct fib6_nh *nh)
{
	...
	if (rt6_nexthop_info(skb, nh, &flags, true) < 0)
		goto nla_put_failure;
