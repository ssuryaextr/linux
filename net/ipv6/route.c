...
static int rt6_fill_node(struct net *net, struct sk_buff *skb,
			 ...
			 unsigned int flags);
static struct rt6_info *rt6_find_cached_rt(struct fib6_result *res,
...
void fib6_select_path(const struct net *net, struct fib6_result *res,
		      struct flowi6 *fl6, int oif, const struct sk_buff *skb,
		      int strict)
{
	struct fib6_info *sibling, *next_sibling;
	struct fib6_info *match = res->f6i;

	if ((!match->fib6_nsiblings && !match->nh) || fl6->flowi6_oif)
		goto out;
	...
	if (unlikely(match->nh)) {
		if (nexthop_is_blackhole(match->nh)) {
			res->fib6_type = RTN_BLACKHOLE;
			res->fib6_flags |= RTF_REJECT;
			res->nh = fib6_info_nh(match);
		} else if (nexthop_is_multipath(match->nh)) {
...
static int rt6_nh_dev_match(struct fib6_nh *nh, void *_arg)
{
			     ...
			     const struct in6_addr *saddr, int oif, int flags)
{
	...
	if (!oif && ipv6_addr_any(saddr)) {
		if (unlikely(rt->nh)) {
			if (nexthop_is_blackhole(rt->nh) && !oif) {
				res->fib6_flags |= RTF_REJECT;
				res->fib6_type = RTN_BLACKHOLE;
				res->nh = fib6_info_nh(rt);
				return;
			}
			/* rt->nh as a dev/gw covered by fib6_info_nh */
		}
		nh = fib6_info_nh(rt);
		if (!(nh->fib_nh_flags & RTNH_F_DEAD))
			goto out;
	...
	for (sprt = rt; sprt; sprt = rcu_dereference(sprt->fib6_next)) {
		if (unlikely(sprt->nh)) {
			...
			if (nexthop_is_blackhole(sprt->nh))
				continue;

			if (nexthop_for_each_fib6_nh(rt->nh, rt6_nh_dev_match,
						     &arg)) {
				nh = arg.nh;
				res->f6i = sprt;
				goto out;
 ...
 */
static enum rt6_nud_state rt6_check_neigh(const struct fib6_nh *nh)
{
	enum rt6_nud_state ret = RT6_NUD_FAIL_HARD;
	struct neighbour *neigh;

	rcu_read_lock_bh();
	neigh = __ipv6_neigh_lookup_noref(nh->fib_nh_dev, &nh->fib_nh_gw6);
	if (neigh) {
			   ...
			   int strict)
{
	...
	if ((strict & RT6_LOOKUP_F_REACHABLE) &&
	    !(fib6_flags & RTF_NONEXTHOP) && nh->fib_nh_has_gw) {
...
static bool find_match(struct fib6_nh *fib6_nh, u32 fib6_flags,
		       int oif, int strict, int *mpri, bool *do_rr)
{
	...
	m = rt6_score_route(fib6_nh, fib6_flags, oif, strict);
			   ...
			   int oif, int strict, bool *do_rr, int *mpri)
{
	...
	for (rt = rt_start;
		...
		if (unlikely(rt->nh)) {
			...
			if (nexthop_for_each_fib6_nh(rt->nh, rt6_nh_find_match,
						     &arg)) {
				match = true;
				nh = arg.nh;
			}
			...
			nh = fib6_info_nh(rt);
			if (find_match(nh, rt->fib6_flags, oif, strict, mpri,
				       do_rr))
				match = true;
		}
		if (match) {
...
static bool rt6_is_gw_or_nonexthop(struct fib6_result *res)
{
	return (res->f6i->fib6_flags & RTF_NONEXTHOP) || res->nh->fib_nh_has_gw;
}
...
static void ip6_rt_copy_init(struct rt6_info *rt, struct fib6_result *res)
{
...
/* called with rcu_lock held */
static struct rt6_info *ip6_create_rt_rcu(struct fib6_result *res)
{
	struct fib6_info *rt = res->f6i;
	struct net_device *dev;
...
restart:
	...
	fib6_select_path(net, &res, fl6, fl6->flowi6_oif, skb, flags);
...
static struct rt6_info *ip6_rt_cache_alloc(struct fib6_result *res,
					   const struct in6_addr *daddr,
					   const struct in6_addr *saddr)
{
	struct fib6_info *ort = res->f6i;
...
static struct rt6_info *ip6_rt_pcpu_alloc(struct fib6_result *res)
{
	struct fib6_info *rt = res->f6i;
...
static struct rt6_info *rt6_get_pcpu_route(struct fib6_result *res)
{
			     ...
			     const struct fib6_nh *nh)
{
	...
	if (rt->fib6_pmtu) {
		...
		rcu_read_lock();
		idev = __in6_dev_get(nh->fib_nh_dev);
		mtu = idev->cnf.mtu6;
		rcu_read_unlock();
	}
				...
				struct fib6_result *res)
{
	 ...
	 */
	if (dst_metric_raw(&nrt->dst, RTAX_MTU) >= fib6_mtu(ort, res->nh)) {
		err = -EINVAL;
		goto out;
	}
...
static int rt6_remove_exception_rt(struct rt6_info *rt)
{
	...
	nh = fib6_info_nh(from);
	return fib6_nh_remove_exception(nh, from->fib6_src.plen, rt);
}
...
static void rt6_update_exception_stamp_rt(struct rt6_info *rt)
{
	...
	nh = fib6_info_nh(from);
	fib6_nh_update_exception(nh, from->fib6_src.plen, rt);
unlock:
	rcu_read_unlock();
}
			...
			unsigned long now)
{
	...
	nh = fib6_info_nh(rt);
	if (!rcu_access_pointer(nh->nh_common.nhc_exceptions))
		goto out;

	spin_lock(&rt6_exception_lock);
	bucket = rcu_dereference_protected(nh->nh_common.nhc_exceptions,
				    lockdep_is_held(&rt6_exception_lock));
			       ...
			       const struct sk_buff *skb, int flags)
{
	...
	fib6_select_path(net, &res, fl6, oif, skb, strict);
...
static bool ip6_redirect_nh_match(struct fib6_result *res,
				  struct flowi6 *fl6,
				  ...
				  struct rt6_info **ret)
{
	...
	if (nh->fib_nh_flags & RTNH_F_DEAD || !nh->fib_nh_has_gw ||
					   ...
					   struct net_device *dev)
{
	...
	for_each_fib6_node_rt_rcu(fn) {
		const struct fib6_nh *nh = fib6_info_nh(rt);

		if (nh->fib_nh_dev->ifindex != ifindex)
			continue;
		if (!(rt->fib6_flags & RTF_ROUTEINFO) || !nh->fib_nh_has_gw)
			continue;
		if (!ipv6_addr_equal(&nh->fib_nh_gw6, gwaddr))
			continue;
		if (!fib6_info_hold_safe(rt))
			continue;
		break;
...
static int fib6_clean_tohost(struct fib6_info *rt, void *arg)
{
	...
	if (((rt->fib6_flags & RTF_RA_ROUTER) == RTF_RA_ROUTER) &&
	    nh->fib_nh_has_gw && ipv6_addr_equal(gateway, &nh->fib_nh_gw6)) {
		return -1;
	}
...
static bool rt6_is_dead(struct fib6_info *rt)
{
	const struct fib6_nh *fib6_nh = fib6_info_nh(rt);

	if (fib6_nh->fib_nh_flags & RTNH_F_DEAD ||
	    (fib6_nh->fib_nh_flags & RTNH_F_LINKDOWN &&
	     ip6_ignore_linkdown(fib6_nh->fib_nh_dev)))
		return true;

	return false;
}

static int rt6_multipath_total_weight(struct fib6_info *rt)
{
	const struct fib6_nh *fib6_nh = fib6_info_nh(rt);
	struct fib6_info *iter;
	int total = 0;
	...
	if (!rt6_is_dead(rt))
		total += fib6_nh->fib_nh_weight;

	list_for_each_entry(iter, &rt->fib6_siblings, fib6_siblings) {
		if (!rt6_is_dead(iter)) {
			fib6_nh = fib6_info_nh(iter);
			total += fib6_nh->fib_nh_weight;
		}
	}
...
static void rt6_upper_bound_set(struct fib6_info *rt, int *weight, int total)
{
	...
	atomic_set(&fib6_nh->fib_nh_upper_bound, upper_bound);
}
...
static int fib6_ifup(struct fib6_info *rt, void *p_arg)
{
	...
	if (rt != net->ipv6.fib6_null_entry && !rt->nh &&
	    nh->fib_nh_dev == arg->dev) {
		nh->fib_nh_flags &= ~arg->nh_flags;
...
static bool rt6_multipath_uses_dev(struct fib6_info *rt,
				   const struct net_device *dev)
{
	struct fib6_nh *nh = fib6_info_nh(rt);
	struct fib6_info *iter;
...
static unsigned int rt6_multipath_dead_count(struct fib6_info *rt,
				 ...
				 unsigned int *flags)
{
	if (nexthop_is_multipath(nh)) {
		...
		if (nexthop_mpath_fill_node(skb, nh, false))
			goto nla_put_failure;
			 ...
			 unsigned int flags)
{
	...
	if (rt6) {
		...
		if (rt6_fill_node_nexthop(skb, rt->nh, &rtm->rtm_flags) < 0)
			goto nla_put_failure;
	} else {
...
static bool fib6_info_uses_dev(struct fib6_info *f6i,
			       const struct net_device *dev)
{
	struct fib6_nh *nh = fib6_info_nh(f6i);
		    ...
		    struct nl_info *info)
{
	...
	/* workaround until in-kernel notifier is implemented and
	 * supported
	 */
