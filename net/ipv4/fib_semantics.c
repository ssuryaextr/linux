...
#define for_nexthops(fi) {						\
	int nhsel; struct fib_nh_common *nhc;				\
	for (nhsel = 0, nhc = fib_info_nhc((fi), 0);			\
	     nhsel < fib_info_num_path((fi));				\
	     nhsel++, nhc = fib_info_nhc((fi), nhsel))

#define change_nexthops(fi) {						\
	int nhsel; struct fib_nh *nexthop_nh;				\
	for (nhsel = 0,	nexthop_nh = fib_info_nh((fi), 0);		\
	     nhsel < fib_info_num_path((fi));				\
	     nhsel++, nexthop_nh = fib_info_nh((fi), nhsel))
...
#define for_nexthops(fi) {						\
	...
	for (nhsel = 0, nhc = fib_info_nhc((fi), 0); nhsel < 1; nhsel++)

#define change_nexthops(fi) {						\
	int nhsel;							\
	struct fib_nh *nexthop_nh = fib_info_nh((fi), 0);		\
...
static inline int nh_comp(struct fib_info *fi, struct fib_info *ofi)
{
	for_nexthops(fi) {
		...
		if (nhc->nhc_oif != onhc->nhc_oif ||
		    nhc->nhc_family != onhc->nhc_family ||
		    nhc->nhc_scope != onhc->nhc_scope ||
#ifdef CONFIG_IP_ROUTE_MULTIPATH
		    nhc->nhc_weight != onhc->nhc_weight ||
#endif
		    lwtunnel_cmp_encap(nhc->nhc_lwtstate, onhc->nhc_lwtstate) ||
		    ((nhc->nhc_flags ^ onhc->nhc_flags) & ~RTNH_COMPARE_MASK))
			return -1;
		...
		if (nhc->nhc_family == AF_INET) {
			...
			nh = container_of(nhc, struct fib_nh, nh_common);
			onh = container_of(onhc, struct fib_nh, nh_common);
			if (nh->nh_tclassid != onh->nh_tclassid)
				return -1;
		}
#endif
	} endfor_nexthops(fi);
...
static void fib_rebalance(struct fib_info *fi)
{
	...
	for_nexthops(fi) {
		...
		if (nhc->nhc_flags & RTNH_F_DEAD) {
			upper_bound = -1;
		} else if (ip_ignore_linkdown(nhc->nhc_dev) &&
			   nhc->nhc_flags & RTNH_F_LINKDOWN) {
			upper_bound = -1;
		} else {
			w += nhc->nhc_weight;
		...
		atomic_set(&nhc->nhc_upper_bound, upper_bound);
	} endfor_nexthops(fi);
}
#else /* CONFIG_IP_ROUTE_MULTIPATH */
		 ...
		 struct netlink_ext_ack *extack)
{
...
#ifdef CONFIG_IP_ROUTE_CLASSID
		if (cfg->fc_flow && cfg->fc_flow != nh->nh_tclassid)
			return 1;
#endif
		if ((cfg->fc_oif && cfg->fc_oif != nh->fib_nh_oif) ||
		    (cfg->fc_gw_family &&
		     cfg->fc_gw_family != nh->fib_nh_family))
			return 1;
	...
	for_nexthops(fi) {
		...
		if (rtnh->rtnh_ifindex && rtnh->rtnh_ifindex != nhc->nhc_oif)
			return 1;
		...
		if (attrlen > 0) {
			...
			if (nla) {
				if (nhc->nhc_family != AF_INET ||
				    nla_get_in_addr(nla) != nhc->nhc_gw.ipv4)
					return 1;
			} else if (nlav) {
		     ...
		     unsigned int *flags, bool skip_oif, bool for_ipv4)
{
	...
	if (nh->nhc_has_gw) {
		...
		case AF_INET:
			...
			break;
		case AF_INET6:
			/* ipv4 route with ipv6 nexthop uses RTA_VIA */
			if (for_ipv4) {
				...
				memcpy(via->rtvia_addr, &nh->nhc_gw.ipv6, alen);
	...
	*flags |= (nh->nhc_flags & RTNH_F_ONLINK);
	...
	if (nh->nhc_lwtstate &&
	    lwtunnel_fill_encap(skb, nh->nhc_lwtstate,
		  ...
		  struct fib_info *fi, unsigned int flags)
{
	...
	if (nhs == 1) {
		...
		if (fib_nexthop_info(skb, nhc, &flags, false, true) < 0)
			goto nla_put_failure;
...
#ifdef CONFIG_IP_ROUTE_MULTIPATH
static bool fib_good_nh(const struct fib_nh_common *nhc)
{
	int state = NUD_REACHABLE;

	if (nhc->nhc_scope == RT_SCOPE_LINK) {
		struct neighbour *n = NULL;
...
void fib_select_multipath(struct fib_result *res, int hash)
{
	...
	for_nexthops(fi) {
		...
		if (hash > atomic_read(&nhc->nhc_upper_bound))
			continue;
