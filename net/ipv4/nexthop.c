...
{
	size_t len = nhg->num_nh * sizeof(struct nh_group);
...
{
	struct nh_group *nhg = rtnl_dereference(nh->nh_grp);
	size_t sz = sizeof(struct nh_group) * nhg->num_nh;
...
static size_t nh_nlmsg_size_single(struct nexthop *nh)
{
	...
	switch (nhi->family) {
	case AF_INET:
		if (nhi->fib_nh.fib_nh_has_gw)
			sz += nla_total_size(4);  /* NHA_GATEWAY */
		break;

	case AF_INET6:
		/* NHA_GATEWAY */
		if (nhi->fib6_nh.fib_nh_has_gw)
			       ...
			       struct netlink_ext_ack *extack)
{
	...
	nhi = rtnl_dereference(nh->nh_info);
	if (scope == RT_SCOPE_HOST && nhi->fib_nhc.nhc_has_gw) {
		NL_SET_ERR_MSG(extack,
		      ...
		      struct netlink_ext_ack *extack)
{
	...
	if (nh->is_group) {
		...
		for (i = 0; i < nhg->num_nh; ++i) {
			...
			err = nexthop_check_scope(nhge->nh, scope, extack);
			if (err)
				goto out;
		}
	} else {
			    ...
			    struct netlink_ext_ack *extack)
{
	...
	err = nlmsg_parse_strict(nlh, sizeof(*nhm), tb, NHA_MAX,
